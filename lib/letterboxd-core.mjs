...
// Parse Letterboxd profile-page HTML for lifetime stats. The page renders
// a series of <h4 class="profile-statistic">…</h4> blocks, each containing a
// <span class="value">N</span> and a <span class="definition…">LABEL</span>.
// We match by LABEL text (case-insensitive) so capitalization changes don't
// silently break us. Returns null per-field if its label is missing, or the
// whole function returns null on falsy/non-string input.
export function parseProfileStats(html) {
  if (!html || typeof html !== "string") return null;
  const findByLabel = (label) => {
    const re = new RegExp(
      `<span class="value">([\\d,]+)</span>\\s*<span class="definition[^"]*">${label}</span>`,
      "i",
    );
  ...
  return {
    filmsTotal: findByLabel("Films"),
    thisYear: findByLabel("This year"),
    lists: findByLabel("Lists"),
    following: findByLabel("Following"),
    followers: findByLabel("Followers"),
  };
}
...
// Pick the highest-rated film from the list. Ties broken by recency
// (films are newest-first in the list, so first match wins).
function pickFavorite(films) {
  let best = null;
  for (const f of films) {
    if (typeof f.rating !== "number") continue;
    if (!best || f.rating > best.rating) best = f;
  }
  return best;
}

function deriveStats(films, now) {
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const thisYear = today.getFullYear();
  const thisMonth = today.getMonth();
  let thisYearCount = 0;
  let thisMonthCount = 0;
  let fiveStarThisYear = 0;
  let likedCount = 0;
  let rewatchCount = 0;
  let ratedSum = 0;
  let ratedN = 0;
  ...
  for (const f of films) {
    ...
    if (d) {
      if (d.getFullYear() === thisYear) {
        thisYearCount++;
        if (d.getMonth() === thisMonth) thisMonthCount++;
        if (f.rating === 5.0) fiveStarThisYear++;
      }
    }
    if (f.liked) likedCount++;
    if (f.rewatch) rewatchCount++;
    if (typeof f.rating === "number") { ratedSum += f.rating; ratedN++; }
    if (typeof f.year === "number") years.push(f.year);
  }
  return {
    totalInWindow: films.length,
    thisYear: thisYearCount,
    thisMonth: thisMonthCount,
    fiveStarThisYear,
    topDecade: computeTopDecade(years),
    avgRating: ratedN ? Math.round((ratedSum / ratedN) * 10) / 10 : null,
    likedCount,
    rewatchCount,
  };
}

export async function buildMovieData({
  user,
  cachePath,
  fetchImpl = fetch,
  now = Date.now,
  ...
  try {
    if (cachePath && fs.existsSync(cachePath)) {
      ...
      if (cached.cacheVersion !== CACHE_VERSION) cached = null;
    }
  } catch { cached = null; }
  if (cached && nowVal - cached.fetchedAt < ttlMs) {
    return cached.data;
  }
  ...
  try {
    ...
    const films = parseLetterboxdRss(xml);

    // Profile-page lifetime stats. Letterboxd 403s the default fetch UA.
    // Failure is non-fatal — falls through with profileStats = null.
    let profileStats = null;
    try {
      const profileUrl = `https://letterboxd.com/${user}/`;
      const pRes = await fetchImpl(profileUrl, {
        headers: { "User-Agent": "Mozilla/5.0 (compatible; doyled-it-bot/1.0)" },
      });
      if (pRes.ok) {
        const html = await pRes.text();
        const parsed = parseProfileStats(html);
        if (parsed && (parsed.filmsTotal || parsed.thisYear || parsed.lists || parsed.following || parsed.followers)) {
          profileStats = parsed;
        }
      }
    } catch (e) {
      console.warn(`letterboxd: profile fetch failed (${e.message})`);
    }

    const nowDate = new Date(nowVal);
    const calendar = computeCalendar(films, nowDate);
    const trendByWeek = computeTrendByWeek(films, nowDate);
    const ratingHistogram = computeRatingHistogram(films);
    const data = {
      ...
      hero: {
        latest: films[0] ?? null,
        favoriteRecent: pickFavorite(films),
      },
      profileStats,
      stats: deriveStats(films, nowDate),
      ratingHistogram,
      histMax: ratingHistogram.reduce((m, b) => (b.count > m ? b.count : m), 0),
      calendar,
      calendarMax: calendar.reduce((m, c) => (c.count > m ? c.count : m), 0),
      trendByWeek,
      trendMax: trendByWeek.reduce((m, b) => (b.count > m ? b.count : m), 0),
      error: null,
    };
    if (cachePath) {
      fs.mkdirSync(path.dirname(cachePath), { recursive: true });
      fs.writeFileSync(cachePath, JSON.stringify({ cacheVersion: CACHE_VERSION, fetchedAt: nowVal, data }));
    }
    return data;
  } catch (err) {
    console.warn(`letterboxd: fetch failed (${err.message})`);
    ...
    return {
      ...
      profileUrl: `https://letterboxd.com/${user}/`,
      films: [],
      hero: { latest: null, favoriteRecent: null },
      profileStats: null,
      stats: { totalInWindow: 0, thisYear: 0, thisMonth: 0, fiveStarThisYear: 0, topDecade: null, avgRating: null, likedCount: 0, rewatchCount: 0 },
      ratingHistogram: computeRatingHistogram([]),
      histMax: 0,
      calendar: computeCalendar([], new Date(nowVal)),
      calendarMax: 0,
      trendByWeek: computeTrendByWeek([], new Date(nowVal)),
      trendMax: 0,
      error: err.message,
    };
  }
