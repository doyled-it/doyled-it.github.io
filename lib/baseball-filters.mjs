...
export function computeCalculated(stats) {
  ...
  // TC is definitionally PO+A+E. Recompute rather than trust the stored value:
  // scorekeeping-while-batting occasionally drops the TC bump on an error.
  const TC = (f.PO || 0) + (f.A || 0) + (f.E || 0);
