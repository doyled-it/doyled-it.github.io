...
export async function buildMusicData({
  ...
  try {
    ...
    data.tagCloud = await aggregateGenreCloud({
      artists: (topArtistsAllTimeJson.topartists?.artist || []).slice(0, 30),
      base,
      fetchImpl,
      cache: tagCache,
    });
...
async function aggregateGenreCloud({ artists, base, fetchImpl, cache }) {
  ...
  for (const a of artists) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          tags = (json?.toptags?.tag ?? [])
            .map((t) => ({ name: t.name.toLowerCase(), count: parseInt(t.count ?? "0", 10) }))
            .filter((t) => t.count > 0);
        } else {
          tags = [];
        }
    ...
    // Top 5 tags per artist, weighted by user's playcount × tag affinity.
