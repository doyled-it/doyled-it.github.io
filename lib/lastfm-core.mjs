...
export async function buildMusicData({
  ...
  try {
    ...
    const data = {
      ...
      tagCloud: [], // populated below from artist.gettoptags aggregation
      trendByWeek,
      trendMax,
      activityHeatmap,
      heatMax,
    ...
    // Build a real genre cloud by aggregating each top artist's top tags
    // (user.gettoptags only returns *user-applied* tags, which is empty
    // for anyone who doesn't hand-tag their library).
    const tagCachePath = path.join(path.dirname(cachePath), "artist-tags.json");
    const tagCache = await loadCache(tagCachePath);
    const allTimeArtistList = topArtistsAllTimeJson.topartists?.artist || [];
    const monthArtistList = topArtistsJson.topartists?.artist || [];
    // Combine all-time + month so a brand-new account with mostly recent
    // data still gets meaningful tag coverage. De-dupe by name.
    const seenArtist = new Set();
    const tagSeed = [];
    for (const a of [...allTimeArtistList, ...monthArtistList]) {
      const k = (a?.name ?? "").toLowerCase();
      if (!k || seenArtist.has(k)) continue;
      seenArtist.add(k);
      tagSeed.push(a);
      if (tagSeed.length >= 30) break;
    }
    data.tagCloud = await aggregateGenreCloud({
      artists: tagSeed,
      base,
      fetchImpl,
      cache: tagCache,
    });
    saveCache(tagCachePath, tagCache);
    console.log(`lastfm: tag cloud built from ${tagSeed.length} artists → ${data.tagCloud.length} tags`);
...
// Aggregate genre tags across the user's top artists. For each artist we
// fetch artist.gettoptags (cached forever, since artist tags are stable),
// then weight each tag by `userPlaycountForArtist × tagCount`. Filter junk
// tags ("seen live", "favourites", etc.) and return the top 18.
const TAG_BLOCKLIST = new Set([
  "seen live", "favorites", "favourites", "favourite", "favorite",
  "owned", "spotify", "all", "love", "loved", "best", "good", "awesome",
  "amazing", "my music", "albums i own", "00s", "10s", "20s", "90s", "80s",
  "70s", "60s", "00", "10", "20", "music", "rock and roll",
]);

async function aggregateGenreCloud({ artists, base, fetchImpl, cache }) {
  ...
  for (const a of artists) {
    const name = a?.name;
    const playcount = Math.max(1, parseInt(a?.playcount ?? "0", 10));
    if (!name) continue;
    const key = name.toLowerCase();
    let tags = cache[key];
    if (!tags) {
      try {
        const url = `${base}&method=artist.gettoptags&artist=${encodeURIComponent(name)}&autocorrect=1`;
        const res = await fetchImpl(url);
        if (res.ok) {
          const json = await res.json();
          // Last.fm returns `tag` as an array OR a single object when
          // there's exactly one result. Normalize.
        ...
        console.warn(`lastfm: artist.gettoptags failed for ${name}: ${err.message}`);
        tags = [];
      }
      cache[key] = tags;
      fetched++;
      await delay(220); // be nice to the API on cold starts
    } else {
      cached++;
    }
    for (const t of tags.slice(0, 5)) {
      if (TAG_BLOCKLIST.has(t.name)) continue;
      const key = normalizeTagKey(t.name);
      const prev = weights.get(key);
      const w = playcount * (t.count / 100);
      if (prev) {
        prev.weight += w;
        // Keep the shortest/cleanest display label (typically the one
        // without spaces or extraneous punctuation).
        if (t.name.length < prev.display.length) prev.display = t.name;
      } else {
        weights.set(key, { display: t.name, weight: w });
      }
    }
  }
  console.log(`lastfm: aggregateGenreCloud — ${fetched} fetched, ${cached} cached, ${weights.size} unique tags`);

  return [...weights.values()]
    .map((v) => ({ name: v.display, count: Math.round(v.weight) }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 18);
}

// Collapse "hip-hop" / "hip hop" / "hiphop" → "hiphop", "r&b" / "rnb" →
// "rnb", etc., so visually-identical tags don't appear twice in the cloud.
